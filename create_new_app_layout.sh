...
echo "MX_APP_${UPPER1}_VERSION=${APP_VER}" >>.env

echo "copying .env to TEMPLATE.env"
cp .env TEMPLATE.env

echo "update compose and dockerfile"
sed "s/\[\&replace_lower_app_name\&\]/${1}/g;s/\[\&REPLACE_UPPER_APP_NAME\&\]/${UPPER1}/g" template_compose_file > mx-base-${1}-build-extracted-compose.yml
sed "s/\[\&REPLACE-APP-NAME\&\]/${UPPER1}/g;s/\[\&replace-user-name\&\]/${1}1/g;s/\[\&replace-app-name\&\]/${1}/g"  template-build-split-docker > mx-base-${1}-build-split-docker

echo "setup app template"
mkdir -p mx-base-${1}/config
sed "s/\[\&replace_lower_app_name\&\]/${1}/g;s/\[\&REPLACE_UPPER_APP_NAME\&\]/${UPPER1}/g" template_start_script.sh > mx-base-${1}/config/start-${1}.sh
#cp template_start_script.sh  mx-base-${1}/config/start-${1}.sh

chown ${LOCAL_DOCKER_USER}:${MX_SHARED_GROUP_NAME} mx-base-${1}-build-split-docker mx-base-${1}-build-extracted-compose.yml
chown -R ${LOCAL_DOCKER_USER} mx-base-${1}
chgrp -R ${MX_SHARED_GROUP_NAME}  mx-base-${1}

echo "remove any default apps variables/args e.g. GITEA _GITEA_"
find . -type f -exec sed -i "/_GITEA_/d" {} \;
echo
