...
if [[ "$MX_INFO_FILE_LOCATION" != "" ]];then
	USE_CASE_FILE=${MX_INFO_FILE_LOCATION}/.${USE_CASE}.info
else
	USE_CASE_FILE=./.${USE_CASE}.info
fi

KEYDIR=${MX_AGEDIR_KEYS}/.${USE_CASE}
INFDIR=${MX_AGEDIR_INFO}/.${USE_CASE}
...
chgrp ${MX_SHARED_GROUP_NAME} ${INFDIR}/${INFILE} ${KEYDIR}/${KEYFILE}

chmod 440 ${INFDIR}/${INFILE} ${KEYDIR}/${KEYFILE}
chmod 440 ${INFDIR}/${INFILE} ${KEYDIR}/${KEYFILE}

chgrp -R ${MX_SHARED_GROUP_NAME}  ${KEYDIR} ${INFDIR}
