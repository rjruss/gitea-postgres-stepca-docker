...
INST_LOCFILE="${MX_BASE_APP_DIR}/postgresql.conf"
CERT_LOCATION=/home/postgres/cert
CERT_NAME="postdb"

STATUS_FILE="postgres_status"
echo "down" >${MX_DB_SHARED_LOCATION}/${STATUS_FILE}
...
check_step_ca () {
        ...
        for (( i=1; i<=${1}; i++ )); do
                ...
                if [[ $retVal -eq 0 ]];then
                        ...
                        let app_cdur=$(echo $MX_POST_CERT_DUR|sed "s/h//")
...
initialise_app () {
	...
	loginfo "initial setup of application"
        ...
        step ca certificate ${MX_CONTAINER_NAME} ${CERT_LOCATION}/${CERT_NAME}.crt ${CERT_LOCATION}/${CERT_NAME}.key  --not-after ${MX_POST_CERT_DUR} --san ${MX_CONTAINER_NAME} --san $HOST_FQDN  --provisioner-password-file  <(set +x;echo -n `shared_get_info.sh STEP PW`;set -x)
        ...
        set -x
        initdb -D ${MX_BASE_APP_DIR}
        pg_ctl -D ${MX_BASE_APP_DIR} -l logfile start
        sed -i "s/trust$/scram-sha-256/" ${MX_BASE_APP_DIR}/pg_hba.conf
        #https://docs.gitea.com/installation/database-prep create using default US UTF8 set in base container 
        psql -c "CREATE DATABASE $MX_POSTGRES_DB  TEMPLATE template0;"
        set +x;psql -q -c "CREATE USER $MX_POSTGRES_USER WITH PASSWORD '$POSTGRES_PASSWORD';"; set -x
        psql -c "GRANT ALL PRIVILEGES ON DATABASE $MX_POSTGRES_DB TO $MX_POSTGRES_USER;"
        psql -c "ALTER DATABASE $MX_POSTGRES_DB OWNER TO $MX_POSTGRES_USER"
...
#       echo "hostssl    $MX_POSTGRES_DB             $MX_POSTGRES_USER             all               scram-sha-256    clientcert=verify-full" |  tee -a ${MX_BASE_APP_DIR}/pg_hba.conf
        echo "hostssl    $MX_POSTGRES_DB             $MX_POSTGRES_USER             localhost               cert" |  tee -a ${MX_BASE_APP_DIR}/pg_hba.conf
	ORIG_IFS=$IFS
	IFS=,
	for subnet in ${MX_ALLOWED_SUBNET}; do
        #echo "hostssl    $MX_POSTGRES_DB             $MX_POSTGRES_USER             ${MX_ALLOWED_SUBNET}               cert" |  tee -a ${MX_BASE_APP_DIR}/pg_hba.conf
        	echo "hostssl    $MX_POSTGRES_DB             $MX_POSTGRES_USER             ${subnet}               cert" |  tee -a ${MX_BASE_APP_DIR}/pg_hba.conf
	done
	IFS=${ORIG_IFS}
        echo "#host    $MX_POSTGRES_DB             $MX_POSTGRES_USER             all               scram-sha-256" |  tee -a ${MX_BASE_APP_DIR}/pg_hba.conf
        echo "#hostssl    $MX_POSTGRES_DB             $MX_POSTGRES_USER             `hostname -I`               scram-sha-256" |  tee -a ${MX_BASE_APP_DIR}/pg_hba.conf
        sed -i "s/#listen_addresses = 'localhost'/listen_addresses = 'localhost,${HOST_FQDN}'/" ${MX_BASE_APP_DIR}/postgresql.conf
...
startup () {
        ...
        pg_ctl -D ${MX_BASE_APP_DIR} -l logfile stop
        retVal=$?
        if [[ $retVal -eq 0 ]];then
                echo "down" >${MX_DB_SHARED_LOCATION}/${STATUS_FILE}
        else
                echo "unknown" >${MX_DB_SHARED_LOCATION}/${STATUS_FILE}
                logwarn "unknown postgres status, so remove any postmaster.pid file"
                rm -rf ${MX_BASE_APP_DIR}/postmaster.pid
        fi
        loginfo "start database"
        pg_ctl -D ${MX_BASE_APP_DIR} -l logfile start
        retVal=$?
        if [[ $retVal -eq 0 ]];then
                echo "up" >${MX_DB_SHARED_LOCATION}/${STATUS_FILE}
        else
                echo "down"  >${MX_DB_SHARED_LOCATION}/${STATUS_FILE}
...
andcheck () {
        ...
        while true
        do
                step certificate needs-renewal --expires-in ${MX_POST_EXP_CHECK}  ${CERT_LOCATION}/${CERT_NAME}.crt
                retVal=$?
                if [ $retVal -eq 0 ];then
                        renew
                        loginfo "reload database to refresh certificate"
                        pg_ctl reload -D ${MX_BASE_APP_DIR}
                fi
        sleep 10m
        done
...
if [ ! -f ${INST_LOCFILE} ];then
        if check_step_ca 4 10; then
                ...
                startup
                        ...
                        #psql -P pager=off "dbname=$MX_POSTGRES_DB  user=$MX_POSTGRES_USER password=$POSTGRES_PASSWORD" -c "SELECT * FROM pg_catalog.pg_tables;"
                #post_startup_init
                sleep 10
                andcheck
        else
